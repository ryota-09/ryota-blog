...
## AI運用ルール(サブエージェント / モデルルーティング)

### モデルの使い分け
- **調査・検索・定型実行(ジュニアレベルのタスク)** → Sonnetサブエージェントに委譲する:
  - `repo-explorer` — コードベース内の横断調査・影響範囲特定(読み取り専用)
  - `web-researcher` — 技術情報のWeb調査(バージョン照合・出典必須)
  - `test-runner` — lint/型チェック/テスト/ビルドの実行と失敗の一次切り分け(修正はしない)
- **結果の統合・抽象度の高い推論・設計判断・意思決定** → Fable 5で行う。原則メインループが担い、Sonnetエージェントには委譲しない
  - 判断業務を委譲する場合はFable 5指定のエージェントを使う。`code-reviewer`(プロジェクト専用)は `model: fable` を明示指定している(セッションのメインモデルに関わらずFable 5でレビューする)

### 委譲時の注意
- 独立した調査タスクは1メッセージで並列に投げる
- Sonnetサブエージェントは「起動しました/お待ちください」とだけ報告して作業せず終了することがある。各エージェント定義に対策(「あなた自身が実行担当者」の明記)を織り込み済みだが、定義外のアドホックな委譲時も同様の一文をプロンプトに含めること。この報告を受け取ったら作業未実行と判断し、再指示するか自分で実行する
- grep 1回で済む単発調査は委譲せず自分で実行する方が速い

### プロジェクトスキル
- `/quality-check` — 品質ゲート一括実行(lint → tsc → vitest → build)。開発の仕上げに使う
- `/cloudflare-preview` — OpenNext/Cloudflare実機検証。redirects/rewrites等ルーティング変更時は必須
