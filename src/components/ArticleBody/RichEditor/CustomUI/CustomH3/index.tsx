...
const CustomH3 = ({ children, id, ...RestProps }: CustomH3Props) => {
  ...
  return (
    <div className="inline-block relative group cursor-pointer my-3" onClick={handleCopy}>
      ...
      {/* NOTE: コピー後の表示 */}
      <p className={cltw("text-xs content-center px-2 py-0.5 rounded bg-base-color dark:bg-secondary translate-x-full duration-300 absolute -top-[1.5rem] right-14 md:right-0 md:top-[5px] text-white", isClicked ? "inline-block" : "hidden")}>Copied !</p>
