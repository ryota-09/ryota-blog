...
const CustomH2 = ({ children, id, ...RestProps }: CustomH3Props) => {
  ...
  return (
    <div className="py-3 border-b-base-color border-b-2 group cursor-pointer">
      <div className="inline-block relative" onClick={handleCopy}>
        ...
        <i className={cltw("text-lg absolute right-0 top-0 opacity-0 group-hover:opacity-100 z-10 transition group-hover:translate-x-full duration-300 group-hover:text-base-color", isClicked ? "hidden" : "inline")}>#</i>
      </div>
    </div>
  );
};
